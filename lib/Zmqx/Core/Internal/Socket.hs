...
blockUntilCanSend socket =
  blockUntilEvent socket Zmqx.Internal.Bindings.ZMQ_POLLOUT

-- Throws ok errors
blockUntilCanReceive :: Zmq_socket -> IO ()
blockUntilCanReceive socket =
  blockUntilEvent socket Zmqx.Internal.Bindings.ZMQ_POLLIN

-- Throws ok errors
blockUntilEvent :: Zmq_socket -> CShort -> IO ()
blockUntilEvent socket event = do
  fd <- zhs_getsockopt_int socket Zmqx.Internal.Bindings.ZMQ_FD
  let loop = do
        threadWaitRead (fromIntegral @Int @Fd fd)
        events <- zhs_getsockopt_int socket Zmqx.Internal.Bindings.ZMQ_EVENTS
